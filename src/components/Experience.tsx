...
const ExperienceItem = ({ exp }: { exp: typeof experiences[0] }) => {
  return (
    <StaggerItem y={50}>
      <div className="relative pl-8 md:pl-0 mb-24 last:mb-0 group cursor-default">
        <div className="md:grid flex flex-col md:grid-cols-5 gap-4 md:gap-12 relative z-10">
          
          {/* Glowing Timeline Node */}
          <div className="absolute left-px md:left-1/2 md:-translate-x-1/2 top-6 size-2 rounded-full bg-white group-hover:bg-accent group-hover:shadow-accent transition-all duration-500 z-20 flex items-center justify-center">
            <motion.div 
              initial={{ scale: 0 }}
              whileInView={{ scale: 1 }}
              transition={{ duration: 0.4, delay: 0.3 }}
              viewport={{ once: true }}
              className="size-6 rounded-full border border-white/20 group-hover:border-accent/50 absolute" 
            />
          </div>
          
          <div className="md:col-span-2 text-left md:text-right pt-6">
            <span className="text-accent font-display tracking-[0.2em] text-sm md:text-base inline-block md:pr-12 drop-shadow-accent">
              {exp.year}
            </span>
          </div>
...
export default function Experience() {
  return (
    <section className="py-32 px-4 md:px-8 lg:px-16 relative z-10 flex flex-col justify-center min-h-screen">
      <div className="max-w-6xl mx-auto w-full">
        ...
        <div className="relative pt-12 pb-12">
          {/* Central Line */}
          <div className="absolute left-1.25 md:left-1/2 md:-translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-white/10 to-transparent" />
